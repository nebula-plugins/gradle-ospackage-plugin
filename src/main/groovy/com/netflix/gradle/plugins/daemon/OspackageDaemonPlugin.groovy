    ...
    void apply(Project project) {
        ...
        extension.daemons.all { DaemonDefinition definition ->
            ...
            project.tasks.withType(SystemPackagingTask) { SystemPackagingTask task ->
                ...
                // Add postInstall content - file will be generated by template task before execution
                // Use zip to make the provider serializable for configuration cache
                def postInstallFilePathProvider = outputDirProvider.zip(project.provider { POST_INSTALL_TEMPLATE }) { dir, template ->
                    new File(dir, template).absolutePath
                }
                task.exten.postInstallCommands.addAll(postInstallFilePathProvider.map { filePath ->
